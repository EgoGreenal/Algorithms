...
给定$n-1$次多项式$A(x)$，求$B(x)\equiv\ln A(x)\ (\text{mod }x^n)$的$B(x)$
...
```c++
struct SAM{
    int ch[maxn][26],fa[maxn],len[maxn],tot,p;
    SAM(){LCT::Create(tot=p=1,false);}
    inline void Extend(int c){
        int np=++tot; len[np]=len[p]+1,LCT::Create(np,true);
        while (p&&!ch[p][c]) ch[p][c]=np,p=fa[p];
        if (!p) return (void)(fa[np]=1,LCT::Link(np,1),p=np);
        int q=ch[p][c]; if (len[p]+1==len[q]) return (void)(fa[np]=q,LCT::Link(np,q),p=np);
        int nq=++tot; len[nq]=len[p]+1,memcpy(ch[nq],ch[q],104),LCT::Create(nq,false),LCT::Cut(q,fa[q]),fa[nq]=fa[q],LCT::Link(nq,fa[q]),fa[np]=fa[q]=nq,LCT::Link(q,nq),LCT::Link(np,nq);
        while (p&&ch[p][c]==q) ch[p][c]=nq,p=fa[p];
        p=np;
    }
    inline void Query(){
        int p=1;
        for (char *c=s;*c;++c) p=ch[p][*c-'A'];
        if (!p) return (void)puts("0");
        LCT::Make_Root(1),LCT::Access(p),LCT::Splay(p);
        printf("%d\n",LCT::virt_siz[p]+LCT::vis[p]);
        //这里如果是离线的话可以遍历parent树算出siz
    }
}T;
```
...
差分算子$\delta$用来表示$A_{nxt[i]}-A_i$或$A_i-A_{lst[i]}$，从而实现查询后置化，修改快速化
